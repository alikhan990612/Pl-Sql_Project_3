...
(
v_dat_t date,                    -- Конец даты на программе который выбирает пользователь.Тем самым передает их на Jasper отчет -> 31.05.2022
...
i := months_between(to_date(p_dat_t_2, 'dd.mm.rrrr'), to_date(p_dat_t_1, 'dd.mm.rrrr'));                      -- Нахожу разницу месяцов, чтобы указать в цикле количество. Â íàøåì ñëó÷àå 01.06.2022 - 01.01.2022=5 
p_i_dat := p_dat_t_1;
...
where a.kod = 1 and a.naim = '1 êàòåãîðèÿ' and a.kod_raion = nvl(v_raion,a.kod_raion) and a.sector = nvl(v_sector,a.sector)
union all
...
join spr_t$klas_obj b on b.kod = a.klas
where a.kod = 2 and a.naim = '2 êàòåãîðèÿ' and a.kod_raion = nvl(v_raion,a.kod_raion) and a.sector = nvl(v_sector,a.sector)
